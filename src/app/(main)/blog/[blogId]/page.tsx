...
const Page = () => {
  ...
  interface BlogItem { id: number; title: string; description: string }
  const blogContent: BlogItem[] = [{ id: 1, title: "Blog meme generator", description: "Paste your blog content and automaticallyinsert memes from a directory of 1000 +memes " },
  ...
  return (
    <div className='min-h-screen min-w-screen align-middle'>
      {blogContent.map((item: BlogItem) => item.id === Number(id) && (
        <div key={item.id} className='flex flex-col items-center justify-center mx-60 p-4 rounded-lg'>
